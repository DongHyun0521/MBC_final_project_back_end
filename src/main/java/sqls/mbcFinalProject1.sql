...
CREATE TABLE payment_method (
    ...
    payment_method_type VARCHAR(20) NOT NULL,				-- 결제수단 종류 (신용카드, 체크카드, 계좌 등)
...
CREATE TABLE parking_spot (
	...
	is_parked BOOLEAN NOT NULL DEFAULT FALSE	-- 주차가 되어있는지?
);
-- 전기차 충전기
...
CREATE TABLE med_staff (
	...
	mem_id INTEGER NOT NULL
		REFERENCES mem(mem_id) ON DELETE CASCADE,	-- FK
	...
	parking_spot_id INTEGER
		REFERENCES parking_spot(parking_spot_id),	-- FK
...
CREATE TABLE admin_staff (
	...
	admin_dept_id INTEGER
		REFERENCES admin_dept(admin_dept_id),		-- FK
	parking_spot_id INTEGER
		REFERENCES parking_spot(parking_spot_id),	-- FK
	create_time TIMESTAMP NOT NULL DEFAULT now()	-- 가입 일시
);
