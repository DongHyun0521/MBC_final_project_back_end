...
public class AdminController {
	...
	public NoticeDto getNoticeDetail(@PathVariable Long noticeId, HttpServletRequest request,
	...
	public String writeNotice(@ModelAttribute NoticeDto noticeDto, HttpSession session) {
		...
		// 권한 체크
		if (loginId == null || member == null || !adminService.isWonMu(member.getMemId()))
			return "fail";
	...
	public HealthStoryDto getHealthStoryDetail(@PathVariable Long healthStoryId, HttpServletRequest request,
			HttpServletResponse response) {
	...
	public String writeHealthStory(@ModelAttribute HealthStoryDto dto, HttpSession session) {
		...
		MemDto member = memService.getMemberInfo(loginId);
		if (loginId == null || member == null || !adminService.isAdmin(member.getMemId())
				|| !adminService.isPr(member.getMemId()))
			return "fail";
	...
	// [최종 프로젝트 추가] 대시보드 관리자 계정 목록 (시설, 보안, 주차)
